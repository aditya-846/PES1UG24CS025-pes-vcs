...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    if      (strncmp((char*)buf, "blob ",   5) == 0) { if (type_out) *type_out = OBJ_BLOB; }
    else if (strncmp((char*)buf, "tree ",   5) == 0) { if (type_out) *type_out = OBJ_TREE; }
    else if (strncmp((char*)buf, "commit ", 7) == 0) { if (type_out) *type_out = OBJ_COMMIT; }
    ...
    out[data_len] = '\0';
    if (data_out) *data_out = out; else free(out);
